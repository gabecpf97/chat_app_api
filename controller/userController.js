...
const get_user = (req, res, next) => {
    User.findById(req.user._id, "name email rooms contacts")
    .exec((err, theUser) => {
       if (err)
        return next(err);
       res.send({theUser}); 
 ...
 */
const create_user = [
    body('username', 'Username must be longer than 3 letter').trim().isLength({min: 3}).escape(),
    ...
    .withMessage("Password must be longer than 8 character").custom(value => {
        return /\d/.test(value)
